...
def test_loader_uses_capture_id_merge_and_integrity_assertion():
    client = FakeClient()
    ...
    assert "ON target.capture_id = source.capture_id" in query
    ...
    assert len(job_config.query_parameters) == 10
